...
intersection_output_set = nan(3, 6, 3, 20);
F_ex_result_output_set = nan(3, 6, 3, 20);
    ...
    for j = 1:6
        for k = 1:3
            ...
            [theta1_given, theta2_given, theta3_given, theta4_given, ...
                ...
                = deal(...
                angle_tension_input_test_disturbed(1,j,k), angle_tension_input_test_disturbed(2,j,k), ...
                angle_tension_input_test_disturbed(3,j,k), angle_tension_input_test_disturbed(4,j,k), ...
                angle_tension_input_test_disturbed(5,j,k), angle_tension_input_test_disturbed(6,j,k), ...
                angle_tension_input_test_disturbed(7,j,k), angle_tension_input_test_disturbed(8,j,k), ...
                angle_tension_input_test_disturbed(9,j,k), angle_tension_input_test_disturbed(10,j,k));
            if isnan(angle_tension_input_test_disturbed(5, j, k))
                continue
            end
            ...
            lambda_x_min = (0 - r0(1)) / F(1);
            lambda_x_max = (15 - r0(1)) / F(1);
            lambda_y_min = (-5 - r0(2)) / F(2);
            lambda_y_max = (25 - r0(2)) / F(2);
            lambda_z_min = (-5 - r0(3)) / F(3);
            ...
            intersection_output_set(:, j, k, i) = intersection;
            F_ex_result_output_set(:, j, k, i) = F_ex_result;
...
for k = 1:3
    valid_points = ~isnan(P_ex_end_test_points(1, :, k));
    scatter3(P_ex_end_test_points(1, valid_points, k), ...
        P_ex_end_test_points(2, valid_points, k), ...
        P_ex_end_test_points(3, valid_points, k), ...
        'ro', 'filled', 'DisplayName', sprintf('Reference surface points, group %d', k));
end
